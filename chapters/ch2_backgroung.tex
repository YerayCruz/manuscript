...
For small forces $\gamma < \epsilon/a$, particles remain trapped. In the intermediate regime $\epsilon/a < \gamma < \epsilon/b$, rectification occurs as particles can climb the gentle slope but not the steep one, yielding either integer velocities $V = n$ when particles fully relax between cycles (Regime 1), or rational velocities $V = n - 1/(m+1)$ when incomplete relaxation creates a periodic pattern over multiple cycles (Regime 2).


Later, this concept was proven experimentally by Faucheux et al. (1995) who created an "optical thermal ratchet" using foucsed lased beams passing through plate to polarizte in circularly in order to keept the laser constant along the whole circular trap, the way the aboided to create any external force due to the laser, they made the frequency of the laser to fast that the particle finds it difficult to experience any directed force, therefore moving freely. After that they change the intensity of the beam mimicking a ratchet as shown in Figure \ref{fig:faucheuxexperiment} \cite{faucheux1995optical}. They observed how the particles localizes in a minimum when the modulation is turned and for long enough periods of time the particles tend to move either backwards or forwards with equal probability.

\begin{figure}[h]
  \begin{center}
    \includegraphics[width=0.65\textwidth]{figures/FaucheuxExperiment.png}
  \end{center}
  \caption[Faucheux experiment]{When the modulation is on, the beam intensity follows the shape of a potential ratchet with 4 modulations per cycle. When the modulation is off the intensity is constant allowing the particle to diffuse freely. Obtained from \cite{faucheux1995optical}}\label{fig:faucheuxexperiment}
\end{figure}

 There was a different approach done by Lee et al. (2004) that used the same concept using holographic optical trappings and a symmetric potential, consisting of discrete optical tweezers which are the ones functioning as wells \cite{lee2005observation}. 
