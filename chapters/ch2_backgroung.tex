...
The definition of active matter can vary slightly among authors. For instance, granular polar asymmetric vibrated particles are considered to exhibit self-propulsion—a characteristic of active matter—despite the fact that their motion arises from external vibrations \cite{deseigne2010collective}. In contrast, a nearly identical setup with symmetric granular particles placed on a non-symmetric (sawtooth) platform and driven by vertical vibrations, which produces directed motion, is not regarded as active \cite{derenyi1998collective}. Similarly, colloidal particles may behave passively when studied individually, but their collective dynamics can be classified as active matter when interactions among particles of the same kind are considered \cite{soto2014self}.

For the purposes of this thesis, active matter will be defined as systems composed of self-driven units that continuously consume energy from their environment to propel themselves \cite{marchetti2013hydrodynamics, ramaswamy2010mechanics}. It is important to emphasize that this definition applies regardless of whether the particles are artificial or organic. In what follows, we will first present macroscopic examples before turning to the microscopic cases most relevant to this work.

\section{Macroscopic Agents}

It is common to confuse what can be considered active matter and what not. However, examples can be observed at plain sight in the macroscopic world. A flock of birds, for instance, is considered active matter because each bird is capable of self-propulsion. Unlike microscopic systems, they are not significantly affected by the thermal noise of their medium—in this case, air. A computational study by Reynolds (1987) modeled flocking behavior in birds by treating each individual as an autonomous agent whose trajectory was influenced by its local environment and neighboring individuals \cite{reynolds1987flocks}.
...
\begin{figure}
  \begin{center}
    \includegraphics[width=0.95\textwidth]{figures/macroscopicagents.png}
  \end{center}
...
To begin, let us review some fundamentals. The key difference of active and passive ones, is that passive undergo mere diffusion motion, whereas active follows stochastic differential equations accordnig to \cite{volpe2014simulation}:
\begin{align}
...
\begin{figure}[h]
  \begin{center}
    \includegraphics[width=0.85\textwidth]{figures/msdmicroscopicagents.png}
  \end{center}
...
\begin{figure}[h]
  \begin{center}
    \includegraphics[width=0.90\textwidth]{figures/randomwalk.png}
  \end{center}
